...
MicData  micBase=0;
int      inTap=0, wasTapped=0, intensity=255;
uint16_t ID=0;
int32_t  m_diff=0;
...
// handler for toggle message
byte toggleHandler(void)
{
    if(thisChunk == NULL) { 
        return 0;
    }
...
// toggle neighbor colors
void toggleNeighbors(void)
{
    int x;
    ...
    for( x=0; x<NUM_PORTS; x++ ) {
        ...
        if(cChunk != NULL) {
            if( sendMessageToPort(cChunk, x, &buf, 1, toggleHandler, (GenericHandler)&freeThisChunk) == 0 ) {
                freeChunk(cChunk);
            }
        }
    }
