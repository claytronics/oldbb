# 1 "/home/dcampbel/Research/blinkyBocksHardware/build/src-bobby/meldinterp-runtime/meldvm.bb"
#include "../sim/block.h"
//#include "bb.h"
...
void enqueueNewTuple(tuple_t tuple, record_type isNew)
{
		if (TYPE_IS_STRATIFIED(TUPLE_TYPE(tuple))) {
			p_enqueue( (this()->newStratTuples) , TYPE_STRATIFICATION_ROUND(TUPLE_TYPE(tuple)), tuple, NULL, isNew);
		}
		else {
			queue_enqueue( (this()->newTuples) , tuple, isNew);
		}
}
...
void enqueue_face(NodeID neighbor, meld_int face, int isNew)
{
		...
		if (neighbor <= 0) {
				...
				SET_TUPLE_FIELD(tuple, 0, & (this()->blockId) );
...
void meldMain(void)
{
		 (this()->blockId)  = getGUID();

		// init stuff
		 (this()->tuples)  = calloc(NUM_TYPES, sizeof(tuple_queue));
		 (this()->newTuples)  = calloc(1, sizeof(tuple_queue));
		 (this()->newStratTuples)  = calloc(1, sizeof(tuple_pqueue));
		 (this()->oldTuples)  = calloc(NUM_TYPES, sizeof(tuple_t));
		 (this()->delayedTuples)  = calloc(1, sizeof(tuple_pqueue));
		 (this()->proved)  = calloc(NUM_TYPES, sizeof(meld_int));
		memset( (this()->receivedTuples) , 0, sizeof(tuple_queue) * NUM_PORTS);
		...
		while(1) {
				...
				if(!queue_is_empty( (this()->newTuples) )) {
						int isNew = 0;
						tuple_t tuple = queue_dequeue( (this()->newTuples) , &isNew);

						tuple_handle(tuple, isNew,  (this()->reg) );
				}
				else if (!p_empty( (this()->delayedTuples) ) && p_peek( (this()->delayedTuples) )->priority <= getTime()) {
						tuple_pentry *entry = p_dequeue( (this()->delayedTuples) );

						tuple_send(entry->tuple, entry->rt, 0, entry->records.count);
						free(entry);
				} else if (!(p_empty( (this()->newStratTuples) ))) {
						tuple_pentry *entry = p_dequeue( (this()->newStratTuples) );
						tuple_handle(entry->tuple, entry->records.count,  (this()->reg) );
				...
				for (i = 0; i < NUM_PORTS; i++) {
						...
						while(!queue_is_empty(&( (this()->receivedTuples) [i]))) {
						  tuple_t tuple = queue_dequeue(& (this()->receivedTuples) [i], NULL);
						  enqueueNewTuple(tuple, (record_type)-1);
						}
...
{
  tuple_t rcvdTuple = (tuple_t) (this()->thisChunk) ->data;
  tuple_t tuple;
  size_t tuple_size = TYPE_SIZE(TUPLE_TYPE(rcvdTuple));

  tuple = malloc(tuple_size);
  memcpy(tuple, rcvdTuple, tuple_size);
  queue_enqueue(& (this()->receivedTuples) [faceNum( (this()->thisChunk) )], tuple, (record_type)isNew);
...
void tuple_send(tuple_t tuple, void *rt, meld_int delay, int isNew)
{
		...
		if (delay > 0) {
				p_enqueue( (this()->delayedTuples) , getTime() + delay, tuple, rt, (record_type) isNew);
				return;
		}

		NodeID target = MELD_NODE_ID(GET_TUPLE_FIELD(tuple, 0));
		if (target ==  (this()->blockId) ) {
				enqueueNewTuple(tuple, (record_type) isNew);
		}
		else {
				int face = -1;
