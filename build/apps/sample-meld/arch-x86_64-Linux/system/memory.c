# 1 "/home/dcampbel/Research/blinkyBocksHardware/build/src-bobby/system/memory.bb"
#include "../sim/block.h"
// memory.c
//
...
void initializeMemory(void)
{
    ...
    {
         (this()->rxChunks) [i].status = CHUNK_FREE;
    }

    // clear all status bits for receive chunks
    for( i=0; i<NUM_TXCHUNKS; i++ )
    {
         (this()->txChunks) [i].status = CHUNK_FREE;
    }
...
Chunk* getSystemChunk(byte which)
{
    ...
    if(which == RXCHUNK)
      {
	current =  (this()->rxChunks) ;

	i = NUM_RXCHUNKS-1;
      }
    else
      {
	current =  (this()->txChunks) ;
