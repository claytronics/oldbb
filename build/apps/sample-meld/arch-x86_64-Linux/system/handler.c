# 1 "/home/dcampbel/Research/blinkyBocksHardware/build/src-bobby/system/handler.bb"
#include "../sim/block.h"
#include "handler.h"
...
void initHandlers(void){
  int i;
   (this()->system) .HandlerMask=0;
  for( i=0; i<NUM_HANDLERS; i++){
     (this()->system) .HandlerTable[i]=NULL;
  }
}
...
void executeHandlers(void)
{
	if( (this()->system) .HandlerMask)
	{
		...
		{
			if( (this()->system) .HandlerMask & j)
			{
				// clear mask bit1
				 (this()->system) .HandlerMask &= ~j;

				// execute callback
				(*( (this()->system) .HandlerTable[i]))();
			}
...
int triggerHandler(Event e)
{
	if(e < NUM_HANDLERS)
	{
		if( (this()->system) .HandlerTable[e] == NULL)
		{
			return 0;
		}

		 (this()->system) .HandlerMask |= (uint32_t)1 << e;
		return 1;
	}
...
int registerHandler(Event e, GenericHandler callback)
{
	if(e < NUM_HANDLERS)
	{
		if( (this()->system) .HandlerTable[e] == NULL)
		{
			 (this()->system) .HandlerTable[e] = callback;
			return 1;
		}
...
int unregisterHandler(Event e)
{
	if(e < NUM_HANDLERS)
	{
		if( (this()->system) .HandlerTable[e] == NULL)
		{
		 ...
		 (this()->system) .HandlerTable[e] = NULL;
		return 1;
	}
