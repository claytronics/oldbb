...
byte getNeighborCount()
{
	...
	{
		if( (this()->thisNeighborhood) .n[i] != VACANT)
		{
			count++;
		}
	}
...
Uid down(void) { return  (this()->thisNeighborhood) .n[DOWN]; }
Uid up(void) { return  (this()->thisNeighborhood) .n[UP]; }
Uid north(void) { return  (this()->thisNeighborhood) .n[NORTH]; }
Uid south(void) { return  (this()->thisNeighborhood) .n[SOUTH]; }
Uid east(void) { return  (this()->thisNeighborhood) .n[EAST]; }
Uid west(void) { return  (this()->thisNeighborhood) .n[WEST]; }
...
#error Invalid DEBUG option chosen - use a face enum.
	...
	if(p < NUM_PORTS)
	{
	    ...
	    if( (this()->thisNeighborhood) .n[p] != b)
	    {
	         (this()->thisNeighborhood) .n[p] = b;
...
void neighborScanCB(void)
{
  ...
  if(chunkResponseType( (this()->thisChunk) ) == MSG_RESP_ACK)
    {
      ( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]).period = NEIGHBOR_HANDSHAKE_TIMEOUT_PERIOD;
    }
  // else failure
  else
    {
      ( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]).period = NEIGHBOR_SCAN_PERIOD;
    }

  // re-enable timer with potentially new period
  enableTimer( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]);
...
void neighborScan(void)
{
  ...
  disableTimer( (this()->ttNeighbor) [p]);
  if( (this()->ttNeighbor) [p].t.state != INACTIVE)
    {
      deregisterTimeout(&( (this()->ttNeighbor) [p].t));
    }
...
void neighborKeepaliveCB(void)
{
    ...
    {
        restartScan(faceNum( (this()->thisChunk) ));
    }
...
void neighborKeepalive(void)
{
  ...
  GUIDIntoChar(getGUID(), &(buf[2]));
  GUIDIntoChar( (this()->thisNeighborhood) .n[ (this()->thisTimeout) ->arg], &(buf[4]));

  // prevent timer from calling this again, message callback will reenable as necessary
  disableTimer( (this()->ttNeighbor) [ (this()->thisTimeout) ->arg]);

  // send message - CB will either re-add timer, or restart scanning for neighbors
  sendSystemMessage( (this()->thisTimeout) ->arg, buf, 6, RES_SYS_HANDLER, (GenericHandler)&neighborKeepaliveCB);
}
...
void neighborHandshakeCB(void)
{
    ...
    if(chunkResponseType( (this()->thisChunk) ) == MSG_RESP_ACK)
    {
      ...
      Uid neighbor = charToGUID(&( (this()->thisChunk) ->data[4]));

      // re-add timer for keepalive purposes
       (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].period = NEIGHBOR_KEEPALIVE_PERIOD;
      if( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].t.state != INACTIVE)
	{
	  deregisterTimeout(&( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].t));
	}
       (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].t.callback = (GenericHandler)&neighborKeepalive;
      enableTimer( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]);

      // add new neighbor to neighborhood
      updateNeighbor(faceNum( (this()->thisChunk) ), neighbor);
...
byte handleNeighborMessage(void)
{
    ...
    switch( (this()->thisChunk) ->data[1])
    {
      ...
      case MSG_NEIGHBOR_START:
      {
	    ...
	    if(( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]).period != NEIGHBOR_HANDSHAKE_TIMEOUT_PERIOD)
	    {
	      ...
	      GUIDIntoChar(getGUID(), &(buf[2]));
	      buf[4] =  (this()->thisChunk) ->data[2];
	      buf[5] =  (this()->thisChunk) ->data[3];
	      ...
	      updateNeighbor(faceNum( (this()->thisChunk) ), VACANT);

	      // stop any imminent timeouts such as neighborscanning
	      disableTimer( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]);
	      if( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].t.state != INACTIVE)
		{
		  deregisterTimeout(&( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )].t));
		}
	      ...
	      sendSystemMessage(faceNum( (this()->thisChunk) ), buf, 6, RES_SYS_HANDLER, (GenericHandler)&neighborHandshakeCB);
	    }
      ...
      case MSG_NEIGHBOR_RESPONSE:
      {
	    ...
	    if(( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]).period == NEIGHBOR_HANDSHAKE_TIMEOUT_PERIOD)
	    {
	        ...
	        neighbor = charToGUID(&( (this()->thisChunk) ->data[2]));
	        me = charToGUID(&( (this()->thisChunk) ->data[4]));
      ...
      case MSG_NEIGHBOR_KEEPALIVE:
      {
	    ...
	    if(( (this()->ttNeighbor) [faceNum( (this()->thisChunk) )]).period == NEIGHBOR_KEEPALIVE_PERIOD)
	    {
	        Uid me, neighbor;
	        ...
	        if(me != getGUID() || neighbor !=  (this()->thisNeighborhood) .n[faceNum( (this()->thisChunk) )])
	        {
		        restartScan(faceNum( (this()->thisChunk) ));
	        }
	    }
...
void restartScan(PRef i)
{
  ...
  disableTimer( (this()->ttNeighbor) [i]);
  if( (this()->ttNeighbor) [i].t.state != INACTIVE)
    {
      deregisterTimeout(&( (this()->ttNeighbor) [i].t));
    }

  // set up timer to call neighbor scan
  ( (this()->ttNeighbor) [i]).t.arg = i;
  ( (this()->ttNeighbor) [i]).t.callback = (GenericHandler)&neighborScan;
  ( (this()->ttNeighbor) [i]).period = NEIGHBOR_SCAN_PERIOD;
  enableTimer( (this()->ttNeighbor) [i]);
}
...
void initEnsemble(void)
{
	...
	registerTimer(&( (this()->ttNeighbor) [i]));
	disableTimer( (this()->ttNeighbor) [i]);
        restartScan(i);
    }
}
