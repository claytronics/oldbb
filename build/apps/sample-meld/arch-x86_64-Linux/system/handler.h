...
/*RXCIE0-5			:	byte received in hardware uart buffer -> check for ack / escape, push data into chunk, increment storage var						flag SYSTEM_ACK_RECEIVED if necessary						flag SYSTEM_PACKET_RX_COMPLETE if necessary, move to alternate buffer (RX_COMPLETE must clear/reallocate new buffer)TXCIE0-5			:	byte transmit buffer free -> push packet byte (if available) into hardware uart buffer, decrement var						flag SYSTEM_PACKET_TX_COMPLETE if necessarysoftuart			:	check if rx/tx bits have/need to happen, record/twiddle as necessary						byte received/transmit as per hardware uart ISRsRTC					:	increment global time, ~1ms units (0 -> time block most recently gained power)Tx_OVF				:	overflow is configured to trigger when nearest-in-time callback/timeout is neededPCx_INT	6x			:	pin change interrupt - one possible way to altert system of new neighborsPCx_INT	+ TWIx		:	accelerometer interrupt + i2c byte push/pull routineDMAC + EVENT + DAC	:	do the DAC using DMA from some sort of memory space*/
