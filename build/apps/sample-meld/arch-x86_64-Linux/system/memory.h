# 1 "/home/dcampbel/Research/blinkyBocksHardware/build/src-bobby/system/memory.bbh"
#include "../sim/block.h"
// memory.h
//
...
/*    EEPROM Usage example:    In your code:    #include "memory.bbh"    typedef struct _meh    {        char data[10];        uint8_t setting;    } some_struct;    // EEPROM data    uint8_t EEMEM nv_data;    some_struct EEMEM nv_mystruct;    // SRAM data    uint8_t data;    some_struct mystruct;    void myinit()    {        restore(&data, &nv_data, sizeof(uint8_t);        restore(&mystruct, &nv_mystruct, sizeof(some_struct));    }    void mymain()    {        if(something worth remarking on)        {            store(&nv_data, &data, sizeof(uint8_t));        }    }*/
...
/* status byte definition   COMMON    [7]   used (1) / unused (0)   SENDING    [6:3] unused    [2:0] sending on face 0bxxx   RECEIVING    [6]   currently filling (1) / completely filled (0)    [5]   receiving odd (1) / even (0) parity chunk    [4:3] unused    [2:0] received from face 0bxxx*/
