# 1 "/home/dcampbel/Research/blinkyBocksHardware/build/src-bobby/system/led.bb"
#include "../sim/block.h"
#include "led.h"
#include "../hw-api/hwLED.h"

// stolen from: http://www.free-webmaster-tools.com/colorpicker.htm
//enum {RED, ORANGE, YELLOW, GREEN, BLUE, INDIGO, VIOLET, WHITE, AQUA, FUCHSIA, LAWNGREEN, LIGHTPINK, LIGHTSLATEGRAY, SADDLEBROWN, GOLD, NUM_COLORS};
uint8_t colors[NUM_COLORS][3] = {
#ifdef BBSIM
{0xFF, 0x00, 0x00}, // red
{0xFF, 0xA5, 0x00}, // orange
{0xFF, 0xFF, 0x00}, // yellow
{0x00, 0xFF, 0x00}, // green
{0x00, 0xFF, 0xFF}, // aqua
{0x00, 0x00, 0xFF}, // blue
{0xFF, 0xFF, 0xFF}, // white
{0xFF, 0x00, 0xFF}, // purple
{0xFF, 0x00, 0x2D}, // pink
#else
{0xFF, 0x00, 0x00}, // red
{0xE1, 0x2D, 0x00}, // orange
{0xFF, 0xEB, 0x00}, // yellow
{0x00, 0xFF, 0x00}, // green
{0x00, 0xFF, 0xFF}, // aqua
{0x00, 0x00, 0xFF}, // blue
{0xFF, 0xFF, 0xFF}, // white
{0xFF, 0x00, 0xFF}, // purple
{0xFF, 0x00, 0x2D}, // pink
#endif
//{0x7C, 0xFC, 0x00}, // lawngrean
//{0xFF, 0xB6, 0xC1}, // lightpink
//{0x77, 0x88, 0x99}, // light slate gray
//{0x8B, 0x45, 0x13}, // saddlebrown
//{0xFF, 0xD7, 0x00}, // gold
//{0x4B, 0x00, 0xB0}, // indigo
//{0xEE, 0x82, 0xEE}, // violet
};

// byte 		currentRGB[3];
// Color 		currentColor;
// Intensity 	currentIntensity = INTENSITY_MAX;

Color getColor()
{
	return  (this()->currentColor) ;
}

Color getNextColor()
{
	Color tmp =  (this()->currentColor) ;

	tmp++;

	if(tmp >= NUM_COLORS)
	{
		tmp = 0;
	}

	return tmp;
}

void setColor(Color c)
{
	 (this()->currentColor)  = c;

	setLED(colors[c][0], colors[c][1], colors[c][2],  (this()->currentIntensity) );
}

Color setNextColor()
{
	Color tmp = getNextColor();

	setColor(tmp);

	return  (this()->currentColor) ;
}

void setLED(byte r, byte g, byte b, Intensity i)
{
	setHWLED(r,g,b,i);
}

void setIntensity(Intensity i)
{
	 (this()->currentIntensity)  = i;

	setLED( (this()->currentRGB) [0],  (this()->currentRGB) [1],  (this()->currentRGB) [2], i);
}

Intensity getIntensity()
{
	return  (this()->currentIntensity) ;
}

void initLED()
{
	initHWLED();
}
