 ...
 *
 *  @brief Instantiates a link within a structure, allowing that 
 *         structure to be collected into a queue created with Q_NEW_HEAD. 
 *
 *  Usage: <br>
...
/** @def Q_STATIC_INIT
 *  
 *  @brief Statically initializes a queue head.
 **/
 ...
 *  @param LINK_NAME Name of the link organizing the queue
 *
 *  @return The element before Q_ELEM, or NULL if there is no previous element
 **/
 ...
 *  or if Q_ELEM is not a member of Q_HEAD's queue, the behavior of this macro
 *  is undefined.
 *
...
#define Q_ENDFOREACH(Q_HEAD)			\
  ;

/** @def Q_JOIN(Q_HEAD_1, Q_HEAD_2, LINK_NAME)
 *  
 *  @brief Puts all of the elements in Q_HEAD_2 into the end of Q_HEAD_1.
 *  
 *  This function splices Q_HEAD_2 onto the tail of Q_HEAD_1, assuming
 *  the heads are of the same type.  Q_HEAD_2 will refer to an empty
 *  queue after execution.
 *  
 *  @param Q_HEAD_1 Pointer to the head of the destination queue.
 *  @param Q_HEAD_2 Pointer to the head of the queue of elements to append.
 *  @param LINK_NAME The name of the link used to organize the two queues.
 **/

#define Q_JOIN(Q_HEAD_1,Q_HEAD_2,LINK_NAME)				\
  {									\
  if ((Q_HEAD_2)->head != NULL)						\
    {									\
      (Q_HEAD_2)->head->LINK_NAME.prev = (Q_HEAD_1)->tail;		\
      if ((Q_HEAD_1)->tail != NULL)					\
	(Q_HEAD_1)->tail->LINK_NAME.next = (Q_HEAD_2)->head;		\
      (Q_HEAD_2)->head = NULL;						\
      (Q_HEAD_2)->tail = NULL;						\
    }									\
  }

/** @def Q_SHALLOW_COPY(Q_HEAD_1, Q_HEAD_2)
 *
 *  @brief Makes a shallow copy of Q_HEAD_1 into Q_HEAD_2
 *  @param Q_HEAD_1 Pointer to the head of the source queue
 *  @param Q_HEAD_2 Pointer to the head of the destination queue
 *  @return Void.
 */
#define Q_SHALLOW_COPY(Q_HEAD_1,Q_HEAD_2)				\
  {								   	\
    (Q_HEAD_2)->head = (Q_HEAD_1)->head;				\
    (Q_HEAD_2)->tail = (Q_HEAD_1)->tail;				\
  }									\
    
/** @def Q_IS_EMPTY(Q_HEAD)
 *  
 *  @brief Return 1, if the Q is empty; 0, otherwise.
 *  @param Q_HEAD Pointer to the head of the queue to check.
 **/

#define Q_IS_EMPTY(Q_HEAD) ((Q_HEAD)->head == NULL)
