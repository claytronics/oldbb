...
threaddef #define CLOCK_CHUNK_RETRANSMISSION_PERIOD 1000
...
threadvar uint16_t minId;
	...
	threadvar byte notWaitingForAnyAnswer;
...
/*#ifdef LOG_DEBUG
		...
		PRINT_BBSIM("%u GO_MSG: stp %u, role %u, valig_gradient %u, id %u, val %u, dist %u\n", getGUID(), recv_step, recv_role, recv_valid_gradient, recv_id, recv_value, recv_distance);
		
		#ifdef LOG_DEBUG
			char s[50];
			snprintf(s, 50*sizeof(char), "GOs%ur%ui%uv%ud%uvg%us",recv_step, recv_role, recv_id, recv_value, recv_distance, recv_valid_gradient);
			s[49] = '\0';
			printDebug(s);
		#endif
		...
		/*#ifdef LOG_DEBUG
			snprintf(s, 50*sizeof(char), "GOc:%u",consistency);
			s[49] = '\0';
			printDebug(s);
		#endif*/
		...
		#ifdef LOG_DEBUG
			snprintf(s, 50*sizeof(char), "GOcp:%u",cmp_res);
			s[49] = '\0';
			printDebug(s);
		#endif
...
broadcastGoMsg(PRef p, byte s, byte r,  uint16_t vg,  uint16_t v, uint16_t id, uint16_t d)
{
	...
	PRINT_BBSIM("%u GO SENT, %u %u %u %u\n", getGUID(), data[0], data[1], data[2], data[3]);
	#ifdef LOG_DEBUG
		char str[50];
		snprintf(str, 50*sizeof(char), "SGOs%ur%ui%uv%ud%uvg%us",s, r, id, v, d, vg);
		str[49] = '\0';
		printDebug(str);
	#endif
...
handleBackMsg(Chunk *c) {
		...
		PRINT_BBSIM("%u BACK_MSG: id %u, val %u, dist %u\n", getGUID(), recv_id, recv_value, recv_distance);

		#ifdef LOG_DEBUG
			char s[50];
			snprintf(s, 50*sizeof(char), "BKi%uv%ud%u",recv_id, recv_value, recv_distance);
			s[49] = '\0';
			printDebug(s);
		#endif
...
stepConsistency(byte re, byte s, byte recvre, byte recvs)
{
	...
	#ifdef LOG_DEBUG
		char str[50];
		snprintf(str, 50*sizeof(char), "cons:re%us%urre%urs%ures%u", re, s, recvre, recvs, res);
		str[49] = '\0';
		printDebug(str);
	#endif
	return res;
